 ...
 */
    ...
    function parseBarcode(barcode, lotLen) {
        var i = 0, // counter
         ...
         */
        function identifyAI(codestring,lotLen) {
            ...
            function parseVariableLength(ai, title) {
                ...
                if (posOfFNC === -1) { //we've got the last element of the barcode
                    if (ai == "10" && lotLen !== undefined && lotLen.trim() != "") { //批号
                        elementToReturn.data = codestring.slice(offSet, parseInt(lotLen) + offSet);
                        codestringToReturn = codestring.replace("10" + elementToReturn.data, "")
                    } else {
        ...
        while (restOfBarcode.length > 0) {
            try {
                firstElement = identifyAI(restOfBarcode, lotLen);
